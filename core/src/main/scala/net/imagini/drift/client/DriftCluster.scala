...
object DriftCluster extends App {
  ...
  manager.createTable("addthis", "syncs", "at_id(STRING), vdna_user_uid(UUID), timestamp(LONG)", 10485760, storageType)
  //  manager.createTable("vdna", "events", "user_uid(UUID), timestamp(LONG), type(STRING), url(STRING)", 100000000, storageType)
  manager.createTable("vdna", "pageviews", "user_uid(UUID), timestamp(LONG), url(STRING)", 10485760, storageType)
