...
class StatScanner(val partition: Int, val regions: Map[String,AimPartition]) extends AbstractScanner {
  ...
  override val schema = AimSchema.fromString("table(STRING),region(INT),count(LONG),distinct(LONG),compressed(LONG),uncompressed(LONG)")
  ...
  private val data: SortedMap[String,Array[ByteBuffer]] = SortedMap(regions.map(r => {
        r._1 -> Array( 
            ...
            ByteUtils.wrap(schema.get(2).convert(r._2.count("*").toString)),
            ByteUtils.wrap(schema.get(3).convert(r._2.count("*").toString)),
            ByteUtils.wrap(schema.get(4).convert(r._2.getCompressedSize.toString)),
            ByteUtils.wrap(schema.get(5).convert(r._2.getUncompressedSize.toString))
         )
  }).toSeq:_* )
