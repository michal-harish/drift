...
class PartitionTest extends FlatSpec with Matchers {
  "Stat partition " should " behave as a normal table" in {
    ...
    scanner.nextLine should be("flags\t1\t4\t4\t121\t141") //TODO count(!) count distinct should be different from count(*)
    scanner.nextLine should be("pageviews\t1\t6\t6\t218\t267")
